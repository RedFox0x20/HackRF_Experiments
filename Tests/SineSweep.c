...
int tx_callback(hackrf_transfer* Transfer)
{
    ...
    const double SweepBandwidth = 4000;
    const double HalfSweepBandwidth = SweepBandwidth / 2;
    const double SweepRate = 4;

    for (i = 0; i < Transfer->valid_length/2; i++) 
    {
        n = (n + 1) % SampleRate;
        const double SweepFreq = 
            HalfSweepBandwidth
            + (HalfSweepBandwidth * sin(2*M_PI*n*SweepRate/SampleRate));
